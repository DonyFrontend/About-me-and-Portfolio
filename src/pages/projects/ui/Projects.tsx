...
const Projects = () => {
    ...
    return (
        <div id="projects" className={`w-full flex ${useC('bg-white', 'bg-[#111827]')} justify-center py-20`}>
            <main className={`componentWidth flex flex-col items-center gap-y-16`}>
                ...
                <section className={`flex flex-col gap-y-12`}>
                    {data.data.map((item, index) =>
                        <article key={index} className={`overflow-hidden hover:-translate-y-4 transition-all flex border-[2px] shadow-lg ${C('shadow-gray-500', 'shadow-violet-500')} ${C('border-[3px]', 'border-violet-500')} rounded-[12px]`}>
